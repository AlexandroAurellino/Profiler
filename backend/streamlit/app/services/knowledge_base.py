...
class KnowledgeBase:
    ...
    def reload(self):
        ...
        logger.info("Reloading Knowledge Base from disk...")
        self.__init__() # Re-run init to reload everything

    def add_or_update_course(self, course: CourseMetadata):
        """
        Updates the in-memory list and Writes to courses.yaml.
        """
        # 1. Update In-Memory Raw Data
        # _raw_courses is a List[Dict]
        updated = False
        new_entry = {"code": course.code, "name": course.name, "sks": course.sks}
        
        for i, entry in enumerate(self._raw_courses):
            if entry.get("code") == course.code:
                self._raw_courses[i] = new_entry
                updated = True
                break
        
        if not updated:
            self._raw_courses.append(new_entry)

        # 2. Persist to Disk
        self._save_yaml("courses.yaml", self._raw_courses)
        
        # 3. Rebuild Index
        self._metadata_map = self._build_metadata_map()
        logger.info(f"Course {course.code} saved successfully.")

    def delete_course(self, code: str) -> bool:
        """
        Removes a course from courses.yaml.
        Returns True if found and deleted.
        """
        initial_len = len(self._raw_courses)
        # Filter out the course
        self._raw_courses = [c for c in self._raw_courses if c.get("code") != code]
        
        if len(self._raw_courses) < initial_len:
            self._save_yaml("courses.yaml", self._raw_courses)
            self._metadata_map = self._build_metadata_map()
            return True
        return False

    def update_relevance_rules(self, code: str, c_type: CriteriaType, weights: Dict[str, float]):
        """
        Updates relevance_rules.yaml.
        Structure: SECTION (Type) -> CODE -> { Profile: Weight }
        """
        section = c_type.value # "FOUNDATION" or "COMPETENCY"
        
        # Ensure section exists
        if section not in self._raw_relevance:
            self._raw_relevance[section] = {}
        
        # Update the specific course entry
        # If weights is empty, we might want to delete the rule? 
        # For now, let's assume we just overwrite.
        self._raw_relevance[section][code] = weights
        
        # Persist
        self._save_yaml("relevance_rules.yaml", self._raw_relevance)
        
        # Rebuild Map
        self._relevance_map = self._build_relevance_map()
        logger.info(f"Relevance rules updated for {code}.")

    def _save_yaml(self, filename: str, data: Any):
        """
        Writes data back to YAML.
        WARNING: This removes comments from the original file.
        """
        file_path = settings.DATA_DIR / filename
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                # default_flow_style=False keeps it in block format (readable)
                # sort_keys=False keeps the order we gave it
